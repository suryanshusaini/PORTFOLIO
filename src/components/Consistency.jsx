...
// Constants
// ─────────────────────────────────────────────────────────────────────────────
const GITHUB_USERNAME = import.meta.env.VITE_GITHUB_USERNAME   ?? "suryanshusaini";
const LEETCODE_USER   = import.meta.env.VITE_LEETCODE_USERNAME ?? "Suryanshu_Saini0808";
...
function Consistency() {
  ...
  // LeetCode — stale-while-revalidate + 1-min heartbeat
  useEffect(() => {
    const LC_STATS_KEY   = "lc-stats";
    const LC_CAL_KEY     = "lc-cal";
    const LC_TS_KEY      = "lc-last-fetched";
    const POLL_MS        = 60_000;

    // Hydrate from cache immediately so UI is never blank
    const cachedStats = localStorage.getItem(LC_STATS_KEY);
    const cachedCal   = localStorage.getItem(LC_CAL_KEY);
    if (cachedStats) { try { setLCStats(JSON.parse(cachedStats)); setLCLoading(false); } catch {} }
    if (cachedCal)   { try { setLCCal(JSON.parse(cachedCal));     } catch {} }

    const fetchLC = async () => {
      // Rate-limit guard: skip if fetched < 60 s ago
      const lastFetched = Number(localStorage.getItem(LC_TS_KEY) || 0);
      if (Date.now() - lastFetched < POLL_MS) return;
      ...
      try {
        ...
        if (statsRes.status === 500 || statsRes.status === 403) {
          console.warn(`LeetCode API is currently down (${statsRes.status}). Serving cached data.`);
          // Silently stay with cache — do not set lcError if cache exists
          if (!cachedStats) setLCError(true);
          return;
        }
        ...
        setLCError(false);
        localStorage.setItem(LC_STATS_KEY, JSON.stringify(statsData));
        localStorage.setItem(LC_TS_KEY, String(Date.now()));

        // Calendar (independent — stats success required first)
        ...
        if (calData?.submissionCalendar) {
          ...
          const cal = parseCalendar(parsed);
          setLCCal(cal);
          localStorage.setItem(LC_CAL_KEY, JSON.stringify(cal));
        }
      } catch (err) {
        console.error("LeetCode Fetch Error:", err.message);
        if (!cachedStats) setLCError(true); // only show error if no cache
      } finally {
        setLCLoading(false);
      }
    };
    ...
    fetchLC();
    const interval = setInterval(fetchLC, POLL_MS);
    return () => clearInterval(interval);
  }, []);

  // GitHub repos — stale-while-revalidate + 1-min heartbeat
  useEffect(() => {
    const GH_KEY    = "gh-repos";
    const GH_TS_KEY = "gh-last-fetched";
    const POLL_MS   = 60_000;

    // Hydrate from cache immediately
    const cachedRepos = localStorage.getItem(GH_KEY);
    if (cachedRepos) {
      try { setRepos(JSON.parse(cachedRepos)); setGHLoading(false); } catch {}
    }

    const fetchGH = async () => {
      const lastFetched = Number(localStorage.getItem(GH_TS_KEY) || 0);
      if (Date.now() - lastFetched < POLL_MS) return;
      ...
      try {
        ...
        if (res.status === 403) {
          console.warn("GitHub API rate-limited (403). Serving cached repos.");
          if (!cachedRepos) setGHError(true);
          return;
        }
        ...
        localStorage.setItem(GH_TS_KEY, String(Date.now()));
      } catch (err) {
    ...
    fetchGH();
    const interval = setInterval(fetchGH, POLL_MS);
    return () => clearInterval(interval);
  }, []);
  ...
  const Card = ({ children, delay = 0, glow = "rgba(255,255,255,0.05)", border = "hover:border-neutral-600", className = "" }) => (
    <motion.div
      ...
      whileHover={{ y: -4, boxShadow: `0 0 36px 0 ${glow}` }}
      className={`bg-neutral-900 border border-neutral-800 rounded-xl p-6 flex flex-col ${border} transition-[transform,box-shadow,border-color] duration-200 ease-out will-change-transform cursor-default ${className}`}
    >
      {children}
    </motion.div>
  );
  ...
  return (
    <section id="consistency" className="section-container">
      ...
      <div className="flex flex-col gap-8 w-full" style={{ contain: "layout" }}>
        ...
        >
          ...
          <div className="flex-grow flex items-center justify-center">
            {lcLoading ? (
              ...
              /* ── Service Interruption Fallback ── */
              <div className="flex flex-col items-center gap-6 text-center py-4 w-full">
                {/* Muted orange warning ring */}
                <div className="relative flex items-center justify-center w-16 h-16">
                  <span className="absolute inset-0 rounded-full border border-[#FFA116]/20 animate-ping opacity-40" />
                  <div className="flex items-center justify-center w-16 h-16 rounded-full border border-[#FFA116]/30 bg-[#FFA116]/5">
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-7 h-7 text-[#FFA116]/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                <div className="flex flex-col gap-1">
                  <p className="text-[#FFA116]/80 text-sm font-mono font-semibold tracking-wide">Service Interruption</p>
                  <p className="text-neutral-300 text-sm font-medium">LeetCode Stats Service is temporarily down.</p>
                  <p className="text-neutral-600 text-xs mt-1 font-mono">The upstream API returned a 500 error.</p>
                </div>

                {/* Prominent profile link */}
                <a
                  href={LC_PROFILE}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 px-6 py-2.5 text-sm font-mono font-medium text-[#FFA116] border border-[#FFA116]/40 rounded-lg bg-[#FFA116]/5 hover:bg-[#FFA116]/10 hover:border-[#FFA116]/70 transition-[background,border-color] duration-200 ease-out"
                >
              ...
              <div className="w-full flex flex-col items-center gap-6">
                ...
                <div className="w-full flex items-center gap-8">
                  ...
                  <div className="group relative flex items-center justify-center shrink-0">
                    <svg width="148" height="148" className="-rotate-90 transition-[transform] duration-200 ease-out group-hover:scale-105" style={{ willChange: "transform" }}>
              ...
              <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
                {repos.map(repo => (
                  <a key={repo.id} href={repo.html_url} target="_blank" rel="noopener noreferrer"
                    className="flex flex-col bg-neutral-950 border border-neutral-800 rounded-lg p-4 hover:border-neutral-500 hover:-translate-y-1 transition-[transform,border-color] duration-200 ease-out will-change-transform group">
